...
public sealed class RoomPlannerScenesController : BaseApiController
{
    ...
    [Authorize(Roles = "CUSTOMER,DESIGNER,SALES,ADMIN")]
    [HttpPost("{sceneId:guid}/room-planner/resolve-layout-assets")]
    public async Task<IActionResult> ResolveLayoutAssets(
        Guid sceneId,
        [FromBody] ResolveRoomPlannerLayoutAssetsRequestDto request,
        CancellationToken cancellationToken = default)
    {
        if (!TryGetCurrentUser(out var currentUserId, out var roleName))
        {
        ...
        var result = await _roomPlannerScenes.ResolveLayoutAssetsAsync(
            sceneId,
            request,
            currentUserId,
            roleName,
