...
public sealed class ProjectChatMessageService : IProjectChatMessageService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var index = 0; index < items.Count; index++)
        {
            ...
            var readModel = messageItems[index];
            item.MessageType = (readModel.MessageType ?? ProjectChatMessageType.TEXT).ToString();

            if (readModel.DeletedAt.HasValue)
            {
                item.Content = null;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        response.MessageType = ProjectChatMessageType.TEXT.ToString();
