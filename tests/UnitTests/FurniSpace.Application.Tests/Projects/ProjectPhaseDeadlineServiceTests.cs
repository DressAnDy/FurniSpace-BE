...
public sealed class ProjectPhaseDeadlineServiceTests
{
    ...
    [Fact]
    public async Task UpsertAsync_AssignedSalesCreatesAndUpdatesWithoutDuplicatingRows()
    {
        ...
        var updateResult = await service.UpsertAsync(
            project.ProjectId,
            SalesId,
            new UpsertProjectPhaseDeadlinesRequestDto
            {
        ...
        Assert.Contains(createResult.Data!.Deadlines, item => item is { Phase: ProjectPhaseType.PROPOSAL, Status: "ON_TRACK" });
        Assert.Contains(createResult.Data.Deadlines, item => item is { Phase: ProjectPhaseType.PRODUCTION, Status: "PLANNED" });
        Assert.Equal(2, createdRows.Count);
        Assert.Equal(2, updatedRows.Count);
        Assert.All(updatedRows, row => Assert.Equal(SalesId, row.CreatedBy));
        Assert.All(updatedRows, row => Assert.Equal(SalesId, row.UpdatedBy));
        Assert.Contains(updatedRows, row => row is { Phase: ProjectPhaseType.PROPOSAL, DueDate: { Year: 2026, Month: 9, Day: 11 } });
    }

    [Fact]
    public async Task UpsertAsync_ReturnsBadRequest_WhenProductionDueDateExceedsTarget()
    {
        await using var context = CreateContext();
        var project = await SeedProjectAsync(context, ProjectStatus.IN_CONSULTATION);
        var service = CreateService(context);
        ...
        var result = await service.UpsertAsync(
            ...
            new UpsertProjectPhaseDeadlinesRequestDto
            {
                ...
                ProductionDueDate = new DateOnly(2026, 10, 1)
            });

        Assert.Equal(400, result.Status);
        Assert.Equal("PHASE_DEADLINE_EXCEEDS_TARGET", result.ErrorCode);
    }

    [Fact]
    public async Task UpsertAsync_ReturnsForbidden_WhenRequesterCannotManageProject()
    {
        await using var context = CreateContext();
        var project = await SeedProjectAsync(context, ProjectStatus.IN_CONSULTATION);
        var service = CreateService(context);

        var result = await service.UpsertAsync(project.ProjectId, DesignerId, ValidRequest());

        Assert.Equal(403, result.Status);
    }

    [Fact]
    public async Task UpsertAsync_ReturnsInvalidStatus_WhenProjectIsNotInConsultation()
    {
        await using var context = CreateContext();
        var project = await SeedProjectAsync(context, ProjectStatus.PROPOSAL_CONSULTING);
        var service = CreateService(context);

        var result = await service.UpsertAsync(project.ProjectId, SalesId, ValidRequest());
    ...
    private static UpsertProjectPhaseDeadlinesRequestDto ValidRequest()
    {
